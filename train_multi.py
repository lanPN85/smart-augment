...
from smaug import NetworkA1, NetworkB1, SmartAugmentMulti
from smaug.dataset import SingleAugmentDataset, MultiAugmentDataset
...
def parse_arguments():
    ...
    parser.add_argument('--train-dir', default='data/imdb_split/train')
    parser.add_argument('--val-dir', default='data/imdb_split/val')
    parser.add_argument('--train-cutoff', type=int, default=None)
    parser.add_argument('--val-cutoff', type=int, default=None)
    parser.add_argument('--no-augment', action='store_true')
    parser.add_argument('--data-type', default='imdb', help='feret | cifar10 | imdb')
    ...
    parser.add_argument('--save-dir', default='models/multi')
...
if __name__ == '__main__':
    ...
    train_dataset = MultiAugmentDataset(train_files, train_labels, img_size=bridge.get_img_size(),
    ...
    net_as = [NetworkA1(channels=2 * channels) for _ in range(bridge.get_num_labels())]
    net_b = NetworkB1(channels=channels, flat_length=968,
                      labels=bridge.get_num_labels(), dropout=args.dropout)
    model = SmartAugmentMulti(net_as, net_b, alpha=args.alpha, beta=args.beta, cuda=args.cuda)
